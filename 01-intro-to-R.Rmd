...
_Start by showing an [example of a script](https://github.com/HPCBio/HPCBio-Toolbox/blob/master/general/Salmon_summarizeOutput.R)_
...
`sqrt` and `round`. You can also get functions from packages you download and install in your library (which we'll talk about in a bit), or even write your own.
...
inputs called *arguments* that are separated by commas. Functions often (but not always) return a *value*. A
...
objects or even calls to other functions. Exactly what each argument means differs per function, and must be
...
What if we forget to separate the arguments with a comma?

```{r, results='show', error = TRUE}
round(3.14159 digits=2)
```

Without the comma, R does not recognize it as an argument and throws an error.
...
This is useful when there start to be more arguments. Our example above could be written:
...
picograms (for some reason). 978Mb = 1 picogram. Divide the
...
```{r, purl=FALSE}
glengths2 <- c(glengths, 90) # adding at the end
glengths2 <- c(30, glengths2) # adding at the beginning
glengths2
```
...
factors (`factor`). We will see examples of these later...
